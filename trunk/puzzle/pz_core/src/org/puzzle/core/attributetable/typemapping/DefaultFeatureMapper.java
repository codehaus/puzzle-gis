...
public class DefaultFeatureMapper implements FeatureMapper{
    ...
    public SimpleFeature transform(SimpleFeature feature) {
        ...
        for(final AttributeDescriptor sourceDesc : mapping.keySet()){
            ...
            final List<AttributeDescriptor> links = mapping.get(sourceDesc);
            if(links == null || links.isEmpty()) continue;

            for(final AttributeDescriptor targetDesc : links){
                Object converted = convert(value, sourceDesc, targetDesc);
                if(converted != null){
                    builder.set(targetDesc.getLocalName(), converted);
                }
            }
        }
    ...
    private static Object convert(Object value, AttributeDescriptor source, AttributeDescriptor target){
